...
unsigned int* compute_transition_function(const string& pattern, unsigned int alphabet_size=256) {
    ...
    for (int letter = 0; letter < alphabet_size; letter++) {
        if (letter == pattern[0]) {
            transition_fun[make_2d(0, letter)] = 1;
        }
        else {
            transition_fun[make_2d(0, letter)] = 0;
        }
        for (int i = 1; i <= pattern_len; i++) {
            const int& state = i;
            ...
            auto& next_state = transition_fun[make_2d(state, letter)];
...
vector<unsigned int> finite_automaton_matcher(const string& text, const string& pattern) {
    ...
    auto* transition_fun = compute_transition_function(pattern);
